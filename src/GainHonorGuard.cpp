...
public:
    ...
    {
        if (!GainHonorGuardEnable || player->GetSession()->IsBot() || !player->IsAlive() || player->InArena() || player->HasAura(SPELL_AURA_PLAYER_INACTIVE))
            return;
        if (!killed || (killed && killed->HasAuraType(SPELL_AURA_NO_PVP_CREDIT)))
            return;
